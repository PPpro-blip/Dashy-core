...
/** Anonymous public viewer for a real `shared_assets` row. */
export function PublicStudioShare({ slug }: { slug: string }) {
  const [state, setState] = useState<State>({ status: "loading" });

  useEffect(() => {
    let cancelled = false;
    void getPublicStudioShare(slug)
      .then((asset) => {
  ...
  return (
    <main className="min-h-screen bg-navy px-5 py-6 sm:px-8">
      <header className="mx-auto flex w-full max-w-5xl items-center justify-between gap-4">
        ...
        <span className="flex items-center gap-1.5 rounded-full border border-cyan-400/20 bg-cyan-400/10 px-3 py-1.5 text-[11px] font-semibold text-cyan-200">
          <SparklesIcon className="h-3.5 w-3.5" />Studio share
        </span>
      </header>

      {state.status === "loading" ? (
        <div className="flex min-h-[70vh] items-center justify-center gap-2 text-sm text-zinc-500">
          <LoaderIcon className="h-4 w-4 animate-spin text-cyan-400" />Loading shared image…
        </div>
        ...
        <article className="mx-auto mt-8 grid w-full max-w-5xl gap-6 lg:grid-cols-[minmax(0,1fr)_21rem]">
          <div className="overflow-hidden rounded-2xl border border-white/[0.08] bg-black/30 shadow-2xl shadow-black/25">
            {/* Database record uses the same stable /api/img-proxy source Studio rendered. */}
