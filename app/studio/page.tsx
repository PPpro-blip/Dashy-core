...
function buildDirectUrl(prompt: string, seed: number): string {
...
export default function StudioPage() {
  ...
  const activeJobsRef = useRef<Map<number, { timer: ReturnType<typeof setTimeout>; cancel: () => void }>>(new Map());
  ...
  const generate = useCallback(
    (promptOverride?: string) => {
      ...
      const id = Date.now();
      const directUrl = buildDirectUrl(text, id);
      ...
      let finished = false;
      let proxyImg: HTMLImageElement | null = typeof window !== "undefined" ? new window.Image() : null;
      ...
      const finish = (status: Tile["status"], loadedUrl?: string, usedDirectFallback = false) => {
        if (finished) return;
        finished = true;
        cleanup();
          ...
          prev.map((tile) =>
            tile.id === id
              ? {
                  ...
                  directFallback: usedDirectFallback,
                }
              : tile
          )
        );
      ...
      const timer = setTimeout(() => {
        console.warn(`[Studio] Generation timed out after ${STUDIO_TIMEOUT_MS / 1000}s for prompt: "${text}"`);
        finish("error");
      }, STUDIO_TIMEOUT_MS);
      ...
      // Dual Load Fallback Logic:
      // 1. Try loading via /api/img-proxy?url=...
      // 2. If proxy fails/errors, automatically fallback to direct image URL
      const tryDirectFallback = () => {
        if (finished || typeof window === "undefined") return;
        console.info(`[Studio] Proxy failed. Retrying with direct Pollinations load: ${directUrl}`);
        const fallback = new window.Image();
        directImg = fallback;
        fallback.onload = () => {
          finish("ready", directUrl, true);
        };
        fallback.onerror = () => {
          // If direct load also fails, mark as error
          console.error(`[Studio] Direct Pollinations load also failed for seed ${id}`);
          finish("error");
        };
        ...
        <header className="flex flex-wrap items-end justify-between gap-5 border-b border-white/[0.06] pb-8">
          <div>
            ...
            <p className="mt-2.5 max-w-xl text-sm leading-relaxed text-zinc-400">
              Turn high-level concepts into instant high-resolution imagery with Dashy’s resilient Vision engine and dual-load proxy fallback.
            </p>
          </div>
          ...
          <div className="flex items-center gap-3 rounded-2xl border border-cyan-400/20 bg-cyan-400/[0.07] px-4 py-3 shadow-lg shadow-cyan-950/30 backdrop-blur-md">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[11px] text-cyan-300/70">40s Dual-Load Pipeline</span>
            </div>
          </div>
        </header>
          ...
          <div className="relative z-10 flex flex-col gap-3">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 text-xs text-zinc-500">
              <p className="flex items-center gap-1.5">
                ...
                <span>for new line · Dual-proxy resilient</span>
              </p>
        ...
        {tiles.length === 0 ? (
          <div className="flex flex-col items-center justify-center rounded-3xl border border-dashed border-white/[0.08] bg-white/[0.015] py-20 text-center">
            ...
            <p className="mt-1.5 max-w-md text-xs leading-relaxed text-zinc-500">
              Type a prompt above and press Generate. Images are rendered using Dashy’s resilient 40-second dual-channel pipeline.
            </p>
          </div>
        ) : (
                  ...
                  {tile.status === "ready" && tile.url ? (
                      ...
                      <div className="absolute inset-0 flex flex-col justify-between bg-gradient-to-t from-black/80 via-black/20 to-transparent p-4 opacity-0 transition-opacity group-hover:opacity-100">
                        ...
                        <div className="flex items-center justify-between">
                          <span className="rounded-lg bg-black/60 px-2 py-1 text-[10px] font-medium text-cyan-300 backdrop-blur-md">
                            {tile.directFallback ? "Direct Pollinations" : "Proxied HD"}
                          </span>
                          <button
                            type="button"
                            onClick={() => generate(tile.prompt)}
                    ...
                    <div className="flex h-full flex-col items-center justify-center gap-3 p-6 text-center">
                      ...
                      <p className="text-xs text-zinc-500">
                        Both the proxy and direct providers were unresponsive.
                      </p>
                      <button
                        type="button"
                        onClick={() => generate(tile.prompt)}
